   ...
   <div class="container-fluid custom-padding">
       <div class="blog_posting_div" onclick="do_show_popup_blog_post()">
            ...
            <div class="blog_posting_text">
                <?php if(isset($_SESSION['LISTYLOGIN'])){?>Post your Confession....<?php } else { echo "<span style='color:red'>Login to post your confessions....</span>"; } ?>
           </div>
        </div>
    </div>
         ...
         <div class="breadcrumb-area">
            <div class="entry-breadcrumb">
               ...
               <?php if($this->uri->segment(2)=="tags"){?>
                   &gt; 
                  <a href="<?php echo base_url();?>confessions" property="itemListElement" typeof="ListItem">
                     <span property="name" class="archive post-rtcl_listing-archive current-item">Confession</span>
                  </a>
               <?php } ?>
               &gt; 
               <a href="<?php echo base_url();?>confessions"  property="itemListElement" typeof="ListItem">
            ...
            <div class="" onclick="<?php if($blog->nsfw ==1){?>show_nsfw_content('<?php echo base_url();?>confession/details/<?php echo $blog->slug;?>');<?php } else { ?>window.location.href='<?php echo base_url();?>confession/details/<?php echo $blog->slug;?>' <?php } ?>" style="cursor: pointer;">
              <ul class="entry-meta conf_" style="display: flex; align-items: center;"> 
                ...
                <li class="entry-admin">
                    <?php if($blog->author != "Anonymous" && 2==3){ ?>
              ...
              ?> </div>
              <?php if($blog->nsfw == 0){?>
            <h3 class="post-title"><a href="<?php echo base_url();?>confession/details/<?php echo $blog->slug;?>"><?php echo $blog->title;?></a></h3>
        <?php } ?>
        <div class="content">
            <?php if($blog->nsfw == 1){?>
...
?>
        ...
        <div class="btn-wrap btn-v2" style="margin-bottom:0; padding-bottom: 0;">
            <div class="button_box">
                <div class="like_dislike likewrap_<?php echo $blog->id;?>">
                    <div class="icon_box_wrap">
                         <?php 
                            $likes = $this->db->query("SELECT * FROM confession_likes WHERE likebg = 1 AND bID = ".$blog->id)->num_rows();
                            if(isset($_SESSION['LISTYLOGIN'])){
                                $check_like = $this->db->query("SELECT * FROM confession_likes WHERE bID = ".$blog->id." AND likebg =1 AND uID = ".user_info()->id)->result_object()[0];
                            }
                        ?>
                        <div class="icon_thumbup like_ <?php echo !empty($check_like)?'selected':'';?>" onclick="do_update_like_dislike('<?php echo $blog->id;?>', 'like_')">
                            ...
                            <div class="text_icon_conf like_count_<?php echo $blog->id;?>"><?php echo $likes; ?></div>
                        </div>

                        <?php 
                                $dislikes = $this->db->query("SELECT * FROM confession_likes WHERE likebg = 2 AND bID = ".$blog->id)->num_rows(); 
                                if(isset($_SESSION['LISTYLOGIN'])){
                                    $check_dislike = $this->db->query("SELECT * FROM confession_likes WHERE bID = ".$blog->id." AND likebg = 2 AND uID = ".user_info()->id)->result_object()[0];
                                }
                            ?>

                        <div class="icon_thumbup dislike_ <?php echo !empty($check_dislike)?'selected':'';?>" onclick="do_update_like_dislike('<?php echo $blog->id;?>', 'dislike_')">
                            ...
                            <div class="text_icon_conf like_count_<?php echo $blog->id;?>"><?php echo $dislikes; ?></div>
                        </div>
                    </div>
                </div>
                <?php /* ?>
                ...
                <?php */ ?>
                <div class="icon_box_wrap" onclick="<?php if($blog->nsfw ==1){?>show_nsfw_content('<?php echo base_url();?>confession/details/<?php echo $blog->slug;?>');<?php } else { ?>window.location.href='<?php echo base_url();?>confession/details/<?php echo $blog->slug;?>' <?php } ?>">
                ...
                <div class="icon_box_wrap" onclick="do_show_share(<?php echo $blog->id;?>)">
                    <div class="svg_conf">
                ...
                <div class="show_share_button_<?php echo $blog->id;?>" style="visibility: hidden;">
                    <div class="sharethis-inline-share-buttons" data-title="<?php echo $blog->title;?>" data-url="<?php echo base_url();?>confession/details/<?php echo $blog->slug;?>"></div>
                </div>
            </div>
        </div>
    </div>
                  ...
                  <div class="entry-content">
                     <div class="rtcl">
                        <div class="row registration-disable registration-not-separate" id="rtcl-user-login-wrapper">
                           <div class="wd100">
                              <h2 class="text-center">POST CONFESSION</h2>
                              ...
                              <div class="button_blog">
                                  <button onclick="close_popup()" class="ff-btn ff-btn-submit ff-btn-md  ff_btn_no_style custom_bord_conf btn ">I'll do it later</button>
                                      <a href="<?php echo base_url();?>post-confession" class="ff-btn ff-btn-submit ff-btn-md item-btn ff_btn_no_style btn btn-primary wd60 postConfessionBtn">Post Your Confession</a>
                                  </a>
                              </div>
                           </div>
                        </div>
...
<script type="text/javascript">
    ...
    function do_update_like_dislike(val, type) {
        if(type == "like_"){
            var url_to_send = 'like_confession';
        } else {
            var url_to_send = 'dislike_confession';
        }
        jQuery.ajax({
                url: '<?php echo base_url();?>nepstate/'+url_to_send+'/'+val+'?loc=ajax',
                cache: false,
                contentType: false,
                processData: false,
                type: 'post',
                success: function(response){
                    ...
                    if(response == 1){
                        ...
                        if(type == "like_"){
                            var dislike_count = jQuery(".likewrap_"+val+' .icon_box_wrap .dislike_ .like_count_'+val);
                            var dislike_svg = jQuery(".likewrap_"+val+" .icon_box_wrap .dislike_");

                            var dilike_count = jQuery(".likewrap_"+val+' .icon_box_wrap .dislike_ .like_count_'+val);
                            var dilike_count_htmlValue = dilike_count.html();
                            if(dilike_count_htmlValue != 0){
                                dislike_svg.removeClass("selected");
                                var htmlValue = dislike_count.html();
                                dislike_count.html((htmlValue - 1));
                            }
                        }

                        if(type == "dislike_"){
                            var dilike_count = jQuery(".likewrap_"+val+' .icon_box_wrap .like_ .like_count_'+val);
                            var dislike_svg = jQuery(".likewrap_"+val+" .icon_box_wrap .like_");

                            var dislike_count = jQuery(".likewrap_"+val+' .icon_box_wrap .like_ .like_count_'+val);
                            var dilike_count_htmlValue = dislike_count.html();
                            console.log(dilike_count_htmlValue);
                            if(dilike_count_htmlValue != 0){
                                dislike_svg.removeClass("selected");
                                var htmlValue = dilike_count.html();
                                dilike_count.html((htmlValue - 1));
                            }
                        }

                        var like_count = jQuery(".likewrap_"+val+' .icon_box_wrap .'+type+' .like_count_'+val);
                        var htmlValue = like_count.html();
                        var new_value = parseInt(htmlValue, 10) + 1;
                        like_count.html((new_value));
                        jQuery(".likewrap_"+val+" .icon_box_wrap ."+type).addClass("selected");
                        return;
                    }
                    ...
                    if(response == 3){
                        var like_count = jQuery(".likewrap_"+val+' .icon_box_wrap .'+type+' .like_count_'+val);
                        var htmlValue = like_count.html();
                        like_count.html((htmlValue - 1));
                        jQuery(".likewrap_"+val+" .icon_box_wrap ."+type).removeClass("selected");
                        return;
                    }
                }
        });
    ...
    function do_show_order_by_sort(val){
        if(val == ""){
           window.location.href = "<?php echo base_url();?>confessions";
        } else {
           window.location.href = "<?php echo base_url();?>confessions?sort="+val;
        }
    }
