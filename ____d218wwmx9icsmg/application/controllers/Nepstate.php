...
class Nepstate extends ADMIN_Controller {
	...
	{   
		...
		setcookie("user_country_id", $countryId, time() + 3600 * 24, "/");
		
		if(isset($_REQUEST['type']) && $_REQUEST['type'] == 'update-country') {
			setcookie('userSelectCountryOrNor', $countryId, time() + 3600 * 24, '/');
		}
	...
	public function cancelCountrySelection()
	{
		if(userSelectOrNotCountry() == false) {
			setcookie('userSelectCountryOrNor', 1, time() + 3600 * 24, '/');
		}
	...
	private function do_send_email($from, $to, $subject, $message, $show=0, $isContact = 1){
		...
		$config = [
			...
			'smtp_port' => 587,
			'smtp_user' => 'noreply.codexperts@gmail.com',
			'smtp_pass' => 'bcfahrsyxtqgdvvu',
			'mailtype'  => 'html',
			'charset'   => 'utf-8',
			'wordwrap'  => true,
			'smtp_crypto' => 'tls',
			...
			if(!isset($_GET['edit'])){
				...
				if(isset($_GET['edit']) || $isFreePlan == 1){
					...
					if(!isset($_GET['edit']) && $isFreePlan == 0){
						...
						$stripeToken = '';
						try {
							Stripe::setApiKey('sk_test_51QgzCzRpAAuqajriqO5qSWVCttNL9HY83iG8S7MMt2sjpuyl2QzArlKxIEv8fRE9EZAuZWqFi02l7HieusimK7yL00jz1mOrM5');
	...
	public function getNotifiationCount()
{
	...
	{			
		...
		\Stripe\Stripe::setApiKey('sk_test_51QgzCzRpAAuqajriqO5qSWVCttNL9HY83iG8S7MMt2sjpuyl2QzArlKxIEv8fRE9EZAuZWqFi02l7HieusimK7yL00jz1mOrM5'); // Stripe Secret Key
