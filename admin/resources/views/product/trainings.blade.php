    ...
    <div class="col-span-12 mt-6">
        <div class="intro-y block sm:flex items-center h-10">
            <h2 class="text-lg font-medium truncate mr-5">
                {{  get_category_title('it-trainings')}} Table
            </h2>
...
<td>{{ $countryName }}</td>
                            ...
                            <td>{{ $training->slug }}</td>
                            <td>
                                @if($training->expiry_date > date('Y-m-d'))
                                    <span class="text-success">Active</span>
                                @elseif($training->expiry_date <= date('Y-m-d'))
                            ...
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center mr-3" href="{{ route('train.show', $training->id) }}?country_id={{ $selectedCountryId }}&status={{ $status }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center">No IT Training available.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
