    ...
    <div class="col-span-12 mt-6">
        <div class="intro-y block sm:flex items-center h-10">
            <h2 class="text-lg font-medium truncate mr-5">
                {{ get_category_title('roomates-rentals') }} Table
            </h2>
            <div class="flex items-center sm:ml-auto mt-3 sm:mt-0">
                <form method="GET" action="{{ route('rentals') }}" class="flex items-center">
...
<th class="whitespace-nowrap">Country</th>
                ...
                <tbody>
                    @forelse ($rentals as $rental)
                        @php
                            $countryName = $countries->firstWhere('id', $rental->country_id)->title ?? 'Unknown Country';
                        @endphp
                        <tr class="intro-x">
                            <td>{{ $rental->title }}</td>
<td>{{ $countryName }}</td>
                            <td>{{ $rental->category }}</td>
                            <td>{{ $rental->sub_cat }}</td>
                            <td>{{ $rental->slug }}</td>
                            <td>
                                @if($rental->expiry_date > date('Y-m-d'))
                                    <span class="text-success">Active</span>
                                @elseif($rental->expiry_date <= date('Y-m-d'))
                                    <span class="text-danger">Expired</span>
                                @endif
                            </td>
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center mr-3" href="{{ route('rental.show', $rental->id) }}?country_id={{ $selectedCountryId }}&status={{ $status }}">
                                        <i class="w-4 h-4 mr-1" data-lucide="eye"></i> View
                                    </a>
                                    <form action="{{ route('destroy.Classified', $rental->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="flex items-center text-danger" onclick="return confirm('Are you sure you want to delete this room?')">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center">No Rooms & Rentals available.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
