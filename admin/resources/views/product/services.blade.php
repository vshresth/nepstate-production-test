    ...
    <div class="col-span-12 mt-6">
        <div class="intro-y block sm:flex items-center h-10">
            <h2 class="text-lg font-medium truncate mr-5">
                {{ get_category_title('services') }} Table
            </h2>
...
<td>{{ $countryName }}</td>
                            ...
                            <td>{{ $service->slug }}</td>
                            <td>
                                @if($service->expiry_date > date('Y-m-d'))
                                    <span class="text-success">Active</span>
                                @elseif($service->expiry_date <= date('Y-m-d'))
                                    <span class="text-danger">Expired</span>
                                @endif
                            </td>
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center mr-3" href="{{ route('service.show', $service->id) }}?country_id={{ $selectedCountryId }}&status={{ $status }}">
