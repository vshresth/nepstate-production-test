...
<th class="whitespace-nowrap">Country</th>
                ...
                <tbody>
                    @forelse ($jobs as $job)
                        @php
                            $countryName = $countries->firstWhere('id', $job->country_id)->title ?? 'Unknown Country';
                        @endphp
                        <tr class="intro-x">
                            <td>{{ $job->title }}</td>
<td>{{ $countryName }}</td>
                            ...
                            <td>
                                ...
                                @elseif($job->expiry_date <= date('Y-m-d'))
                                    <span class="text-danger">Expired</span>
                                @endif
                            </td>
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center mr-3" href="{{ route('job.show', $job->id) }}?country_id={{ $selectedCountryId }}&status={{ $status }}">
                                        <i class="w-4 h-4 mr-1" data-lucide="eye"></i> View
                                    </a>
                                    <form action="{{ route('destroy.Classified', $job->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="flex items-center text-danger" onclick="return confirm('Are you sure you want to delete this job?')">
