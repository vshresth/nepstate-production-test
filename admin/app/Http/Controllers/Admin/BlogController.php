...
class BlogController extends Controller
{
    ...
    {
        $blogs = Blogs::where('is_approved', 1)
    ...
    public function approveBlog($id)
{
    ...
    $user = Users::findOrFail($blog->uID); 

    $blog->is_approved = 1;
    $blog->save();

    Mail::to($user->email)->send(new BlogStatusMail($user->name, 'Approved'));
...
public function reject($id)
{
    ...
    Mail::to($user->email)->send(new BlogStatusMail($user->name, 'Rejected'));

    return redirect()->back()->with('message', 'Blog has been rejected successfully.');
}


    public function approval()
    {
        $blogs = Blogs::whereIn('is_approved', [0, 2])
                  ->where('status', 1)
                  ->orderBy('created_at', 'desc')
                  ->get();
