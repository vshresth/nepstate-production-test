...
class ForumCategoryController extends Controller
{
    ...
    public function destroy(Forums $forum)
    {
        ...
        $forum->delete();
        $getCategoryForums = DB::table('confessions')->where('type', 'forum')->where('cID', $forum->id)->get();

        foreach($getCategoryForums as $forum) {
            DB::table('confession_comment')->where('bID', $forum->id)->delete();
            DB::table('confession_likes')->where('bID', $forum->id)->delete();
            DB::table('confession_views')->where('cID', $forum->id)->where('type', 1)->delete();
            DB::table('confessions')->where('id', $forum->id)->delete();
        }
